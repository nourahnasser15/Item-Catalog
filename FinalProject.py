...
APPLICATION_NAME = "CatalogMakeUp"
...
# 1- return one item for one Catalog in format json
...
# 2- return items for one catalog in format json
...
def deleteCatalog(catalog_id):
    deletedCatalog = session.query(
        Catalog).filter_by(id=catalog_id).one()
    if 'username' not in login_session:
        return redirect('/login')
    if deletedCatalog.user_id != login_session['user_id']:
        return '''<script>function myFunction() {
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash('%s Successfully Deleted' % deletedCatalog.name)
...
def showItems(catalog_id):
    ...
    'user_id']:
        return render_template(
            'publicItems.html', items=items, catalog=catalog, creator=creator)
    else:
        return render_template(
            'showItems.html', items=items, catalog=catalog, creator=creator)
...
# Edit an item
@app.route(
 '/catalog/<int:catalog_id>/item/<int:item_id>/edit/', methods=['GET', 'POST'])
def editItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return '''<script>function myFunction() {alert('You are not authorized
                to edit Items of this catalog. Please create your own catalog
                first.');}</script><body
                onload='myFunction()''>'''
    if request.method == 'POST':
        if request.form['name']:
...
def deleteItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return '''<script>function myFunction() {alert('You are not authorized
                to delete items of this catalog. Please create your own
                catalog first.');}</script><body
                onload='myFunction()''>'''
    if request.method == 'POST':
